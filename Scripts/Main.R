...
train_test_splits = map(rocv_by_crime2$splits, function(split){
  train = analysis(split) %>% #Using analysis will create our training data
    gather(incident, crime_count, -incident_date, -month, -year, -week, -weekday, -Holiday_Christmas:-Holiday_Easter) #Converting data back to long format without the date features
